...
const arrivingOrDeparting = (m, p) => {
  ...
  if (p.dataFilter) {
    // num delays
    if (!p.direction) {
      return `${m.DEPARTING_FLIGHTS_AVERAGE_DELAY_TIME} average delay (minutes) for departing flights`;
    } else {
      return `${m.ARRIVING_FLIGHTS_AVERAGE_DELAY_TIME} average delay (minutes) for arriving flights`;
    }
  } else {
    // avg delay time
    if (!p.direction) {
      return `${m.DEPARTING_FLIGHTS_DELAYS_COUNT} departing flights delayed`;
    } else {
      return `${m.ARRIVING_FLIGHTS_DELAYS_COUNT} arriving flights delayed`;
    }
  }
  ...
  useEffect(() => {
    fetch("/api", {
      ...
      response
        .json()
        .then((query1Data) =>
          setQuery1Data(query1Data.filter((it) => it.FLIGHTYEAR == props.YEAR))
        )
    );
  }, [props.YEAR]);
  ...
  return (
    <ComposableMap data-tip="" projection="geoAlbersUsa">
      <Geographies geography={geoUrl}>
        {({ geographies }) =>
          geographies.map((geo) => {
            const cur = query1Data.find(
              (s) => stateMap.get(s.STATECODE) === geo.id
            );
            return (
              <Geography
                key={geo.rsmKey}
