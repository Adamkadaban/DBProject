...
const arrivingOrDeparting = (m, p) => {
  ...
  if(p.dataFilter){
    ...
    // avg delay time
    if(!p.direction){
      return `${m.DEPARTING_FLIGHTS_AVERAGE_DELAY_TIME} departing flights delayed on average`
    }
    else{
      return `${m.ARRIVING_FLIGHTS_AVERAGE_DELAY_TIME} arriving flights delayed on average`
    }
  }
